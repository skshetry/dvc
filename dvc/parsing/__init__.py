...
class DataResolver:
    def __init__(self, repo: "Repo", wdir: PathInfo, d: dict):
        ...
        if self.tree.exists(to_import):
            ...
            self.global_ctx = Context.load_from(self.tree, relpath(to_import))
        else:
        ...
        for index, item in enumerate(vars_):
            ...
            if isinstance(item, str):
                ...
                if path in skip_imports:
                    continue
                context.merge_from(self.tree, path)
    ...
    def _each_iter(self, context: Context, name: str, in_data, iterable, key):
        ...
        # ? to provide precise overwrite error message for `item`
        # ? and possibly `key`
        check_keys = ["item"]
        ...
        try:
            ...
            if isinstance(exc, MergeError) and exc.key in check_keys:
                raise ResolveError(
    ...
    def _resolve_foreach_data(
        ...
        if isinstance(iterable, str) or not isinstance(
            ...
            raise ResolveError(
                ...
                + type(
                    iterable
                    if not isinstance(iterable, Node)
                    else iterable.value
                ).__name__
            )

        # ? to check if "item" or "key" already exists in the `vars`
        check_keys = ["item"]
        if isinstance(iterable, Mapping):
            check_keys.append("key")
        warn_for = [k for k in check_keys if k in context]
        if warn_for:
            logger.warning(
